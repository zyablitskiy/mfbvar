mcmc_sampler.mfbvar_minn_fsv <- function(x, ...){
  ...
  dimnames(h) <- list(NULL, c(colnames(Y), paste0("fac_", 1:n_fac)), NULL)
  dimnames(facload) <- list(colnames(Y), NULL, NULL)
  rownames(mu) <- colnames(Y)
  rownames(phi) <- rownames(sigma) <- c(colnames(Y), paste0("fac_", 1:n_fac))
